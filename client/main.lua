	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				TriggerServerEvent('wv_shops:buyItem', data.current.item, data.current.value, zone)
			end
...
AddEventHandler('wv_shops:hasEnteredMarker', function(zone)
	currentAction     = 'shop_menu'
	currentActionMsg  = _U('press_menu')
	currentActionData = {zone = zone}
end)

AddEventHandler('wv_shops:hasExitedMarker', function(zone)
	currentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('wv_shops:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			ESX.HideUI()
			TriggerEvent('wv_shops:hasExitedMarker', lastZone)
		end
	Wait(Sleep)
	end
end)
